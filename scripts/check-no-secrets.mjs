...
function collectBuildManifests(directory, files = []) {
  if (!existsSync(directory)) return files;
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    const path = join(directory, entry.name);
    if (entry.isDirectory()) collectBuildManifests(path, files);
    else if (/manifest.*\.json$/i.test(entry.name) || entry.name === 'BUILD_ID') files.push(relative(root, path));
  }
  return files;
}

const candidates = new Set([...trackedFiles(), ...collectBuildManifests(join(root, '.next'))]);
const violations = [];
...
console.log(`Secret-leak gate passed (${candidates.size} tracked files and build manifests checked).`);
