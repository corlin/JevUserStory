...
const navigation = [
  ...
  { label: 'Policies', href: undefined, phase: 'Phase 2' },
  { label: 'Decision Lab', href: undefined, phase: 'Phase 2' },
      ...
      <aside className="app-sidebar">
        ...
        <nav aria-label="主导航" className="app-nav">
          {navigation.map((item) => item.href ? (
            <Link className="nav-item nav-active" href={item.href} key={item.label}>
              <span>{item.label}</span><span className="nav-dot" aria-hidden="true" />
            </Link>
          ) : (
